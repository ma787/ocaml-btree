...
| Il (ks, pls, cn, r, t) -> List.hd ks
| Lf (ks, pls, r, t) -> List.hd ks
...
| Il (v::next, pl::pls, c::cn, r, t) -> (match c1, c with
  ...
  | Il (k1::k1s, p1::p1s, ca::cn1, r1, t1), Il (k3::k3s, p3::p3s, cb::cn3, r3, t3) ->
    if (k1=k3 && p1=p3) then Il (k::v::next, p::pl::pls, c1::c2::cn, r, t)
    else restore (update_node (Il (next, pls, cn, r, t)) k p c1 c2) v pl c
  ...
  if k=v then
    ...
    if leftc then
      if l1 < t then
        ...
        | Il ([], [], c1::[], r1, t1) -> Il ([], [], (delete c1 k)::[], r1, t1)) (* merge children and recursively delete *)
