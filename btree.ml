...
let rec add_items tree k p cn = match tree with
| Lf ([], [], r, t) -> Lf (k::[], p::[], r, t)
| Lf (v::next, pl::pls, r, t) -> Lf(k::v::next, p::pl::pls, r, t)
| Il ([], [], [], r, t) -> 
  if List.length cn < 2 then raise (MalformedTree "internal node must have >1 child")
  else Il (k::[], p::[], cn, r, t)
| Il (v::next, pl::pls, c, r, t) -> Il (k::v::next, p::pl::pls, c @ cn, r, t)
...
| Il (v::next, pl::pls, c1::c2::cn, r, t) -> (* every non-leaf node must have at least 2 children *)
  if List.length(v::next) == 2*t-1 then
    ...
    | Il (k1s, p1::pl1, cn1, r1, t) -> 
      ...
      let m = if l mod 2 == 0 then (l/2)+1 else (l+1)/2 in
      if List.length k1s == 2*t-1 then insert (split c1 tree m) (k, p)
      else insert c1 (k, p)
    | Lf (v1::next1, p1::pl1, r1, t) -> 
      let l = List.length (v1::next1) in
      let m = if l mod 2 == 0 then (l/2)+1 else (l+1)/2 in
      if List.length(v1::next1) == 2*t-1 then insert (split c1 tree m) (k, p)
      else insert c1 (k, p)
    ...
    | Il (k2s, p2::pl2, cn2, r2, t) ->
      let l = List.length (k2s) in
      let m = if l mod 2 == 0 then (l/2)+1 else (l+1)/2 in
      if List.length k2s == 2*t-1 then insert (split c2 tree m) (k, p)
      else insert c2 (k, p)
    | Lf (v2::next2, p2::pl2, r2, t) ->
      let l = List.length (v2::next2) in
      let m = if l mod 2 == 0 then (l/2)+1 else (l+1)/2 in
      if List.length(v2::next2) == 2*t-1 then insert (split c2 tree m) (k, p)
      else insert c2 (k, p)
    | _ -> raise (MalformedTree "internal node must have >1 child")
  else insert (Il (next, pls, c1::c2::cn, r, t)) (k, p)
| _ -> raise (MalformedTree "internal node cannot be empty or without children")

let rec merge parent s1 s2 = match parent with
...
| Il(v::next, pl::pls, c1::c2::cn, r, t) -> 
  ...
  else add_items (merge (Il (next, pls, cn, r, t)) s1 s2) v pl [c1;c2]
